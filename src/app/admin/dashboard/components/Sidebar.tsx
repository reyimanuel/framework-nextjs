...
export default function Sidebar() {
  ...
  const navigation = [
    ...
    { name: "Pengaturan", href: "/admin/settings", icon: MdSettings },
  ]
  ...
  return (
    <>
      ...
      <div
        className={`${collapsed ? "md:w-20" : "md:w-64"} hidden md:flex md:flex-col fixed h-full bg-gradient-to-b from-gray-900 via-blue-900 to-indigo-900 transition-all duration-300 z-30`}
      >
        <div className="flex items-center justify-between h-16 bg-gradient-to-r from-blue-800 via-indigo-800 to-purple-800 px-4">
          <div className="flex items-center space-x-3">
            <Image
              ...
              width={40}
              height={40}
              className="rounded-full bg-white p-1 border-2 border-blue-400"
            />
            {!collapsed && (
              <div>
          ...
          <nav className="flex-1 px-2 py-4 space-y-1">
            {navigation.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon
                    className={`${collapsed ? "mr-0" : "mr-3"} h-5 w-5 transition-all duration-200 ${isActive ? "text-blue-300" : "group-hover:text-blue-400"}`}
                  />
                  {!collapsed && <span>{item.name}</span>}
                  {collapsed && (
                    <div className="absolute left-full ml-6 px-2 py-1 bg-gray-800 rounded-md text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                      {item.name}
                    </div>
                  )}
                </Link>
      ...
      {/* Mobile sidebar */}
      <div className="md:hidden fixed inset-y-0 left-0 w-64 bg-gradient-to-b from-gray-900 via-blue-900 to-indigo-900 z-30 transform -translate-x-full transition-transform">
