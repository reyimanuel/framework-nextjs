...
export default function MemberTable({ refreshTrigger }: MemberTableProps) {
  ...
  const [editMember, setEditMember] = useState<MemberRequest | null>(null)
  ...
  const filteredMembers = members.filter(
    (member) =>
      ...
      (selectedFilter === "Semua" || member.division === selectedFilter)
  )

  const handleEditMember = (member: MemberRequest) => {
    setEditMember(member)
  }

  const handleEditSubmit = async (data: MemberRequest) => {
    ...
    <div className="bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-700">
      ...
      <div className="px-6 py-3 flex flex-col sm:flex-row items-center justify-between border-t border-gray-700">
        <div className="text-sm text-gray-400 mb-2 sm:mb-0">
          ...
          <span className="font-medium">{members.length}</span> pengurus
        </div>
